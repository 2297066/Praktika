...
public class VerleihServiceImplTest
{
    ...
    @Test
    public void testWurdenMedienVerliehen()
    {
    	List<Medium> _medienliste = new ArrayList<Medium>();
    	_medienliste.add(_abbey);
    	_medienliste.add(_bad);
    	_medienliste.add(_shape);
    	_verleihService.verleiheAn(_homer, _medienliste, Datum.heute());
    ...
    @Test
    public void testWerdenMedienZurückGenommen()
    {
    	List<Medium> _medienliste = new ArrayList<Medium>();
    	_medienliste.add(_abbey);
    	_medienliste.add(_bad);
    	_medienliste.add(_shape);
    	_verleihService.verleiheAn(_homer, _medienliste, Datum.heute());
    	_verleihService.nimmZurueck(_medienliste, Datum.heute());
    	assertTrue(_verleihService.mediumImBestand(_abbey));
    	assertTrue(_verleihService.mediumImBestand(_bad));
    	assertTrue(_verleihService.mediumImBestand(_shape));
    }
